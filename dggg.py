...
def get_rank(raw_name, region="na"):
    ...
    lp = soup.find("span", {"class": "lp"}).getText()
    rank = soup.find("div", {"class": "tier-rank"}).getText()
...
def connect():
    ...
    cur.execute(
        """CREATE TABLE IF NOT EXISTS matches(match_id TEXT PRIMARY KEY, duration INTEGER, player TEXT, first_drake INTEGER, first_blood INTEGER, win INTEGER,
...
def champ_winrate(champion, mode=None):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM matches WHERE ? IN (ally_bot, ally_jung, ally_mid, ally_sup, ally_top) AND ? NOT IN (dest_champ, moot_champ)",
        (champion, champion),
    )
    total_ally_games = cur.fetchall()[0][0]
    # get total rows where champion is an ally and player won
    cur.execute(
        "SELECT COUNT(*) FROM matches WHERE win=1 AND ? IN (ally_bot, ally_jung, ally_mid, ally_sup, ally_top) AND ? NOT IN (dest_champ, moot_champ)",
        (champion, champion),
    )
    ...
    cur.execute(
        "SELECT COUNT(*) FROM matches WHERE ? IN (enemy_bot, enemy_jung, enemy_mid, enemy_sup, enemy_top) AND ? NOT IN (dest_champ, moot_champ)",
        (champion, champion),
    )
    total_enemy_games = cur.fetchall()[0][0]
    # get total rows where champion is an enemy and player won
    cur.execute(
        "SELECT COUNT(*) FROM matches WHERE win=0 AND ? IN (enemy_bot, enemy_jung, enemy_mid, enemy_sup, enemy_top) AND ? NOT IN (dest_champ, moot_champ)",
        (champion, champion),
    )
...
def duration_winrate(duration):
    ...
    total_games = cur.fetchall()[0][0]
    cur.execute(
        f"SELECT COUNT(*) FROM matches WHERE duration > ?AND win=1", (duration,)
    )
    wins = cur.fetchall()[0][0]
    winrate = (wins / total_games) * 100
    ...
    for guy in history_targets:
        ...
        while True:
            try:
                ...
                mrm_history = cass.get_match_history(
                ...
                for match in mrm_history:
                    try:
                        match_id = (
                            ...
                            else ("na1_" + str(match.id))
                        )
                        conn = sqlite3.connect(f"./db/match_history.db")
                        cur = conn.cursor()
                        cur.execute(
                            f"SELECT COUNT(*) FROM matches WHERE match_id = ?",
                            (match_id.lower(),),
                        )
                        ...
                        if steven in match.participants:
                            ...
                            ally_team = (
                                match.red_team
                                if steven in match.red_team
                                else match.blue_team
                            )
                            if dest_part.team_position.name != "bot_lane":
                                ally_team_dict["offrole"] = "dest"

                        ally_team_dict["first_drake"] = (
                            1 if ally_team.first_dragon else 0
                        )
                        ally_team_dict["first_blood"] = (
                            1 if ally_team.first_blood else 0
                        )

                        if lucas in match.participants and steven in match.participants:
                            player = "duo"
                            if (
                                moot_part.team_position.name != "bot_lane"
                                and moot_part.team_position.name != "utility"
                            ):
                        ...
                        sql_list = [
                            ...
                            dest_champ,
                            offrole,
                            ally_team_dict["top_lane"],
                            ally_team_dict["jungle"],
                            ally_team_dict["mid_lane"],
                            ally_team_dict["bot_lane"],
                            ally_team_dict["utility"],
                            enemy_team_dict["top_lane"],
                            enemy_team_dict["jungle"],
                            enemy_team_dict["mid_lane"],
                            enemy_team_dict["bot_lane"],
                            enemy_team_dict["utility"],
                        ]
            ...
            if match_count > match_limit:
                ...
                break
            print(
                f"match count is {str(match_count)}, sleeping for {str(5+len(matches))}"
            )
...
def select_champ(search_term: str):
    ...
    selected_champ = [
        match for match in champ_list if re.findall(f"{search_term}.*", match)
    ]
    if not selected_champ:
        selected_champ = [
            match
            for match in champ_list
            if re.findall(f"{search_term}.*", match.replace("'", ""))
        ]
    if not selected_champ:
        for champ in champ_list:
            if len(champ.split()) > 1:
                if (
                    champ.split()[0][0] == search_term[0]
                    and champ.split()[1][0] == search_term[1]
                ):
...
def spec_check(player):
    ...
    match = cass.get_current_match(
        region=cass.data.Region.north_america, summoner=gamer
    )
